...
DWORD WINAPI ThreadReceiveProc(LPVOID n) {
	...
	while (wpData->connected == true) {
		...
		if (WaitCommEvent(wpData->hComm, &CommEvent, 0)) {
			...
			ReadInput(buffer);
			if (buffer[1] == ENQ && wpData->status == IDLE && wpData->sentdEnq == FALSE) {
				OutputDebugString("Received an ENQ");
				control = buffer[0];
				sendAcknowledgment(control);
				wpData->status = RECEIVE_MODE;
				OutputDebugString("going into receive mode!");
				while (wpData->status == RECEIVE_MODE) {
					if (!WaitInput(1000000)) {
						OutputDebugString("didnt receive anything in timeout duration");
						wpData->status = IDLE;
					}
					else {
						OutputDebugString("Received something!");
			...
			else if (buffer[1] == REQ && buffer[0] == wpData->currentSyncByte && wpData->status == SEND_MODE) {
				SetEvent(ackEvent);
